...
### 🃏 供应商卡片
- 支持 **拖动排序**：按住卡片标题区（⋮⋮）拖到目标位置即可，松手自动保存
- 支持 **显示 / 复制完整 SK 秘钥**（默认掩码显示，点"显示"查看明文）
- 支持 **导入 / 导出 JSON**：一键备份或迁移所有供应商配置
...
- 右键托盘 → "退出" 才会真正关闭程序
- 图标纯代码生成，不依赖外部图片文件
- 仅 Windows 桌面版可用；在无桌面依赖环境会自动降级为纯 Web 服务

### 🌙 其他
- 深色 / 浅色模式切换（自动记忆）
- 所有回复强制简体中文
- 界面文字可自由选择复制
- 内置 `/health` 健康检查端点，便于容器探活
...
仓库已把**服务器核心依赖**与**桌面 GUI 依赖**拆分，避免在 NAS / 容器内装上用不到的 GUI 库。

```bash
# 仅 Web 服务（服务器 / 容器 / 飞牛部署，推荐）
pip install -r requirements.txt

# 仅当你要本地桌面窗口模式或打包 exe 时，额外安装桌面依赖
pip install -r requirements-desktop.txt
```

#### 3a. 以桌面窗口模式运行（带原生窗口，仅 Windows 且已装桌面依赖）
```bash
python app.py
```
会启动服务并弹出一个原生桌面窗口（需额外安装 `pip install -r requirements-desktop.txt`）。
...
也可以直接 `python app.py`（在无桌面依赖环境会自动降级为纯 Web 服务）。

#### 3c. Docker / 飞牛 NAS 部署（推荐用于 NAS）

仓库内置 `Dockerfile` 和 `docker-compose.yml`，一行命令即可部署：

```bash
docker compose up -d --build
```

- 服务监听 `8000` 端口，访问 `http://NAS的IP:8000`
- 配置文件持久化到宿主机 `./data` 目录（`providers.json` / `config.json` 等），重启不丢
- 数据目录可通过环境变量 `GATEWAY_DATA_DIR` 指定
- 内置 `/health` 健康检查，飞牛/容器编排可直接探活

飞牛 NAS 上：把本项目放到 NAS 目录，进入目录执行上面的 `docker compose` 命令；或在飞牛"Docker 应用"里导入 `docker-compose.yml`。
