...
if getattr(sys, 'frozen', False):
    ...
    # 支持通过环境变量指定数据目录（Docker/飞牛部署持久化用），缺省回退到本目录
    DATA_DIR = Path(os.environ.get("GATEWAY_DATA_DIR") or Path(__file__).parent)
...
# 这些常量均可通过环境变量覆盖，方便在内存受限设备（如飞牛 NAS）上收紧
MAX_HISTORY_DAYS = int(os.environ.get("GATEWAY_MAX_HISTORY_DAYS", "30"))
MAX_USAGE_DAYS = int(os.environ.get("GATEWAY_MAX_USAGE_DAYS", "30"))
HISTORY_CLEANUP_INTERVAL = 6 * 3600
ONE_MILLION = 1048576
POLL_INTERVAL = int(os.environ.get("GATEWAY_POLL_INTERVAL", "300"))
...
# 是否在回复开头注入 "🤖 提供商 · 模型" 前缀（可在管理面板开关）
SHOW_MODEL_PREFIX = app_config.get("show_model_prefix", True)
...
async def poll_all():
    ...
    # 首次拉取 model details（并发执行，避免 provider 较多时启动缓慢/卡死）
    async def _fetch_details(p):
        try:
    ...
    if providers:
        await asyncio.gather(*[_fetch_details(p) for p in list(providers)], return_exceptions=True)
...
async def lifespan(app: FastAPI):
    ...
    # 内存受限设备（飞牛 NAS 等）收紧连接池与超时，避免慢请求/连接堆积
    http_client = httpx.AsyncClient(
        timeout=httpx.Timeout(30.0, connect=10.0),
        limits=httpx.Limits(
            max_connections=int(os.environ.get("GATEWAY_MAX_CONNECTIONS", "30")),
            max_keepalive_connections=int(os.environ.get("GATEWAY_MAX_KEEPALIVE", "10")),
        ),
    )
...
@app.get("/health")
async def health():
    """健康检查端点（无需鉴权），供容器编排 / 反向代理探活使用"""
    return {"status": "ok", "version": APP_VERSION, "providers": len(providers)}


@app.get("/api/settings/prefix")
async def get_prefix_setting(_=Depends(verify_admin)):
    """获取"回复开头显示模型"开关状态"""
    return {"ok": True, "show_model_prefix": SHOW_MODEL_PREFIX}


class PrefixSettingIn(BaseModel):
    show_model_prefix: bool


@app.put("/api/settings/prefix")
async def set_prefix_setting(data: PrefixSettingIn, _=Depends(verify_admin)):
    """更新"回复开头显示模型"开关，并持久化到 config.json"""
    global SHOW_MODEL_PREFIX
    SHOW_MODEL_PREFIX = bool(data.show_model_prefix)
    cfg = load_config()
    cfg["show_model_prefix"] = SHOW_MODEL_PREFIX
...
# 注意：字面量路由必须在 /api/providers/{name} 等动态路由之前声明，
# 否则 "reorder" 会被当作 {name} 匹配（FastAPI 按声明顺序匹配路由）。
class ReorderProvidersIn(BaseModel):
    names: list[str]


@app.put("/api/providers/reorder")
async def reorder_providers(data: ReorderProvidersIn, _=Depends(verify_admin)):
    """按给定名称顺序重排 providers 列表；未列出的保持原顺序追加到末尾"""
    global providers
    name_order = {name: i for i, name in enumerate(data.names)}
    unnamed_idx = len(data.names)
    async with providers_lock:
        providers.sort(key=lambda p: name_order.get(p["name"], unnamed_idx))
        save_providers(providers)
    return {"ok": True}
...
# ---------- 供应商批量 导入 / 导出 / 排序 / 完整 Key ----------
@app.get("/api/providers/export")
async def export_providers(_=Depends(verify_admin)):
    """导出全部供应商配置（含完整 api_key），用于备份/迁移"""
    return JSONResponse(
        content=providers,
        headers={"Content-Disposition": 'attachment; filename="providers.json"'},
    )


class ImportProvidersIn(BaseModel):
    providers: list[dict]
    mode: str = "merge"  # merge: 按 name 合并覆盖同名; replace: 整体替换


@app.post("/api/providers/import")
async def import_providers(data: ImportProvidersIn, _=Depends(verify_admin)):
    """导入供应商配置。清理后写入 providers.json"""
    global providers
    incoming = data.providers
    if not isinstance(incoming, list):
        raise HTTPException(400, "providers 字段必须是数组")
    cleaned = []
    seen = set()
    for item in incoming:
        ...
        cleaned.append({
            ...
            "free_only": item.get("free_only", True),
        })
    if not cleaned:
        raise HTTPException(400, "没有有效的供应商数据")
    async with providers_lock:
        if data.mode == "replace":
            providers = cleaned
        else:
            incoming_names = {c["name"] for c in cleaned}
            kept = [p for p in providers if p["name"] not in incoming_names]
            kept.extend(cleaned)
            providers = kept
        save_providers(providers)
...
@app.get("/api/providers/{name}/key")
async def get_provider_key(name: str, _=Depends(verify_admin)):
    """返回某个供应商的完整 api_key（供管理面板查看/复制）"""
    for p in providers:
        if p["name"] == name:
            return {"ok": True, "api_key": p.get("api_key", "")}
    raise HTTPException(404, "未找到")
...
async def _try_stream_forward(url, body, headers, key, provider_name, model):
    ...
    async def gen():
        ...
        prefix_done = not SHOW_MODEL_PREFIX  # 开关关闭时跳过前缀注入
...
if __name__ == "__main__":
    ...
    def _has_desktop_deps():
        """检测是否具备桌面窗口运行所需依赖（仅 Windows 桌面版需要）"""
        try:
            import webview  # noqa
            import pystray  # noqa
    ...
    if not _has_desktop_deps():
        # 服务器 / 容器 / Linux 等无桌面环境：纯 Web 模式
        logger.info("desktop deps not found, starting web-only mode on 0.0.0.0:8000")
